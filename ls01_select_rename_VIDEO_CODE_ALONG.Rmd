...
```{r}
yaounde %>% select(age) # Select age
```

By position:

```{r}
yaounde %>% select(3) # Select the 3rd column
```

Multiple variables:

```{r}
yaounde %>% select(age, sex, igg_result) # Select age, sex and igg_result
```
...
```{r, render = reactable_5_rows}
yao %>% select(!c(age, sex, igg_result)) # Drop `age`, `sex` and `igg_result`
```
